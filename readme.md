...
| `headcheck http://broken.host`  | failure: connection refused               | 1         |
| `headcheck http://unknown.host` | failure: unknown host                     | 1         |
...
- Despite the name, since version `2.0.0` headcheck performs a `GET` request, not a `HEAD` request
